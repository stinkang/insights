...
@property (nonatomic, strong) CALayer *selectionIndicatorRoundBoxLayer;
...
- (void)commonInit {
    ...
    if (!(self.selectionStyle == HMSegmentedControlSelectionStyleRoundBox)) {
    ...
    self.selectionIndicatorRoundBoxLayer = [CALayer layer];
...
- (void)drawRect:(CGRect)rect {
    ...
    self.selectionIndicatorRoundBoxLayer.backgroundColor = self.selectionIndicatorColor.CGColor;
    ...
    if (self.selectedSegmentIndex != HMSegmentedControlNoSegment) {
        if (self.selectionStyle == HMSegmentedControlSelectionStyleArrow) {
            if (!self.selectionIndicatorArrowLayer.superlayer) {
               ...
               } else if(self.selectionStyle == HMSegmentedControlSelectionStyleRoundBox) {
                    if (!self.selectionIndicatorRoundBoxLayer.superlayer) {
                        CAShapeLayer *maskLayer = [CAShapeLayer layer];
                        self.selectionIndicatorEdgeInsets = UIEdgeInsetsMake(0.0f, 8.0f, 0.0f, 8.0f);
                        self.selectionIndicatorHeight = 0.0f;
                        self.selectionIndicatorBoxLayer.frame = [self frameForFillerSelectionIndicator];
                        maskLayer.path = [UIBezierPath bezierPathWithRoundedRect: self.selectionIndicatorBoxLayer.bounds cornerRadius:23].CGPath;
...
- (CGRect)frameForFillerSelectionIndicator {
    ...
    if (self.selectionStyle == HMSegmentedControlSelectionStyleRoundBox) {
        return CGRectMake(self.segmentWidth * self.selectedSegmentIndex, 3, self.segmentWidth, CGRectGetHeight(self.frame) - 6);
    }
...
- (void)updateSegmentsRects {
    /*experiment*/
    if (self.selectionStyle == HMSegmentedControlSelectionStyleRoundBox) {
        self.scrollView.contentInset = UIEdgeInsetsMake(0, 18, 0, 0);
    } else {
